within OpenHPL.Examples.AGC_Trollheim;
model TrollheimAGC "Trollheim HPP AGC benchmark: 75 MW to 90 MW load step at t=5 s"
  ...
  OpenHPL.Waterway.Pipe intake(H=20,L=500,D_i=6,D_o=6);
  OpenHPL.Waterway.Pipe penstock(H=300,L=500,D_i=4,D_o=4);
  OpenHPL.ElectroMech.Turbines.Turbine turbine(ValveCapacity=false,H_n=340,Vdot_n=50,u_n=1,ConstEfficiency=true,eta_h=0.90,Pmax=P_base,J=J_each,p=p,enable_nomSpeed=false,enable_P_out=true);
  OpenHPL.Waterway.Pipe discharge(H=2,L=600,D_i=6,D_o=6);
...
initial equation
  ...
  annotation(experiment(StartTime=0,StopTime=65,Tolerance=1e-7,Interval=0.02),
    Documentation(info="<html><h4>TrollheimAGC</h4><p>Trollheim-inspired 150 MW nonlinear hydro frequency-control benchmark. Load steps from 75 MW to 90 MW at t=5 s. The hydraulic backbone is reservoir, intake, penstock, turbine, discharge and tailrace. The controller uses R=50% and Ki=0.10 1/s. See README.md in the same example folder for equations, parameters, initialization, tuning history, and verified Railway/OpenModelica results.</p></html>"));
end TrollheimAGC;
