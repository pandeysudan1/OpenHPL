...
getErrorString();
simulate(OpenHPL.Examples.AGC_Trollheim,startTime=0,stopTime=65,numberOfIntervals=3250,tolerance=1e-7,outputFormat="csv",variableFilter="time|frequency_Hz|mechanicalPower|electricalLoad|guideVane|turbineFlow|surgeLevel|generator.f|governor.gate|governor.x_i|turbine.Wdot_s|turbine.Vdot|loadStep.y|surgeTank.h");
getErrorString();
EOF
...
print('FINAL_surge_m=',float(df.iloc[-1][h]))
print('TROLLHEIM_DATA_START')
for ts in [0,1,4.9,5,5.2,5.5,6,7,10,20,40,65]:
    j=(df.time-ts).abs().idxmin(); r=df.loc[j]
    print(f"{r['time']:.2f},{r[f]:.9f},{r[pm]/1e6:.9f},{r[pl]/1e6:.9f},{r[g]:.9f},{r[q]:.9f},{r[h]:.9f}")
